...
## Decision 019
Date: 2026-09-05
Topic: Release pipeline — tag-triggered GitHub Actions, two artifacts, check-and-notify updates
Decision: Pushing a `v*` tag builds and publishes a signed Android APK and a Linux x64 tarball to a GitHub Release, alongside a small `latest.json` manifest. `.github/workflows/checks.yml` (analyze + test) is the gate, reused by `release.yml` via `workflow_call`. The version ships from `pubspec.yaml`; a `version-guard` job fails the run in seconds if the tag disagrees. Android release builds are signed from `android/key.properties` or CI environment variables, falling back to debug signing locally — and the release job refuses to publish an APK that turns out to be debug-signed. In-app updates are **check-and-notify only**: `UpdateChecker` reads `latest.json` and reports a version and a link, and never downloads or installs.
Reason: Adapted from a working Electron/NSIS/SQL-Server pipeline used on another project. The *shape* transferred — one trigger, a cheap gate before an expensive build, version-from-manifest discipline, `permissions: contents: write`, publish-then-verify-the-assets — but roughly half of that design (bundling a database engine, a custom installer wizard, `electron-updater`'s download-and-restart) has no analogue here and was dropped rather than simulated. LinkDrop has no install-time configuration and no bundled runtime dependency: `CertManager.ensureCertExists` (Decision 015) generates the TLS keypair in-app on first launch, so a packaged build needs no setup step at all. Self-updating was rejected because it cannot be done honestly on either target — a sideloaded APK needs `REQUEST_INSTALL_PACKAGES` plus a FileProvider and a per-update trip through Android's unknown-sources screen, and a tarball has no package manager to hand the work to. `latest.json` is read as a release *asset* rather than through the GitHub REST API because asset downloads escape the API's 60-per-hour unauthenticated rate limit, which is shared per source IP.
Consequences: A release is `pubspec.yaml` bump → commit → annotated tag → push; the tag message is the release notes, and there is no CHANGELOG file. The `+N` build number is Android's `versionCode` and must increase every release. The android job fails until four repository secrets exist (see `docs/RELEASING.md`) — deliberately, since a debug-signed APK can never be upgraded in place by a properly signed one. Losing the keystore means every existing install must be removed by hand. Three prerequisites had to be fixed for the gate to be usable at all: `flutter analyze` exited 1 on `avoid_print` in the root CLI harness scripts (now a file-local ignore, so `lib/` stays covered); `test/ui_golden_test.dart` is tagged `golden` and excluded in CI because golden pixels depend on the host's installed fonts; and `pubspec.lock` is no longer gitignored, since an application whose CI re-resolves every run can break with no commit to blame.

## Decision 020
Date: 2026-09-05
Topic: `package_info_plus` for the running build's version number
Decision: Added `package_info_plus` so `UpdateChecker` can read the version of the *installed* package on both Android and Linux.
Reason: An update check has to compare against what is actually on the device. Dart has no way to read that; the alternatives were a compiled-in constant generated from `pubspec.yaml` (which drifts the moment someone edits one and not the other) or a `--dart-define` (empty in every local build). Both make the check quietly wrong rather than obviously broken, which is the failure mode this feature most needs to avoid.
Consequences: One more dependency, and the only new one this pipeline required. `url_launcher` was deliberately *not* added: on Linux the download link opens through `xdg-open`, reusing the pattern already in `FileReveal`, and on Android the About screen offers the link to copy rather than a button that would quietly do nothing. If a real launcher is wanted later, that is its own decision.
